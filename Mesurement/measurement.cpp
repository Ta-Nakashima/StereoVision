...
int main(int argc, char *argv[])
{
  ...
  while (1){
    ...
    if(cnt == 2){
      ...
      //フレーム数ごとの差分計算
      Diff_Cal(frame_l1, frame_l2, frame_l3, dif1, dif2, &diff1, &result1, &color);
      Diff_Cal(frame_r1, frame_r2, frame_r3, dif1, dif2, &diff2, &result2, &color2);
      ...
      Cleate_Hist(result1, col, row, histl_x, histl_y, &histl_rows, &histl_cols);
      Cleate_Hist(result2, col, row, histr_x, histr_y, &histr_rows, &histr_cols);

      //ヒストグラム平均の描写
      int ave_l1, ave_l2;
      int ave_r1, ave_r2;
      ave_l1 = Hist_Ave(col, histl_x, &histl_rows, 0);
      ave_r1 = Hist_Ave(col, histr_x, &histr_rows, 0);
      ave_l2 = Hist_Ave(row, histl_y, &histl_cols, 1);
      ave_r2 = Hist_Ave(row, histr_y, &histr_cols, 1);
      ...
      Reframe(&ReCmb1, col, row, histl_rows, histl_cols, color, number1);
      Reframe(&ReCmb2, col, row, histr_rows, histr_cols, color2, number2);
...
void Diff_Cal(Mat frame1, Mat frame2, Mat frame3, Mat dif1, Mat dif2,Mat *diff, Mat *result, Mat *color){
  ...
  threshold(*diff, *diff, THRESHOLD, 255, THRESH_BINARY | THRESH_OTSU);    //閾値THRESHOLDで二値化
