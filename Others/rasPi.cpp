...
int main(int argc, char *argv[])
{
  ...
  ofstream histfile("record/" + mp + "_hist.txt");
  #endif
  ...
  while (1){
    ...
    if((num == 1 && cnt == 2) || (num == 2 && cnt == 4) || (num == 3 && cnt == 6)){
      ...
      Cleate_Hist(result1, col, row, histl_x, histl_y, &histl_rows, &histl_cols, outputfile, timefile, histfile);
      Cleate_Hist(result2, col, row, histr_x, histr_y, &histr_rows, &histr_cols, outputfile, timefile, histfile);
    ...
    for(i = 0; i < scan_dir; i++){
      ...
      if((int)pos[i] > ave + ST_HI){
        ...
        for(tmp = 0; tmp < REPETE; tmp++){
          ...
          #ifdef TXT
          outputfile << "S_SLOPE_UP = " << slope << endl;
          #endif
        ...
        if(check == 1){
          ...
          for(tmp = 0; tmp < REPETE; tmp++){
            slope = atan2((double)(pos[i + RANGE] - pos[i]), (double)RANGE);
            #ifdef TXT
            outputfile << "S_SLOPE_DOWN = " << slope << endl;
            #endif
            while(slope < - 0.5){
